    ...
    public async Task<IActionResult> OnPostChangeEmailAsync()
    {
        ...
        if (Input.NewEmail != email)
        {
            ...
            await _emailSender.SendEmailAsync(
                Input.NewEmail,
                "Подтвердите ваш адрес почты",
                $"Пожалуйста, подтвердите свой адрес эл. почты, перейдя по ссылке: <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'></a>.");

            StatusMessage = "Ссылка для подтверждения адрес отправлена на указанный адрес эл. почты.";
            return RedirectToPage();
        }

        StatusMessage = "Ваш адрес эл. почты не изменен.";
        return RedirectToPage();
    }
